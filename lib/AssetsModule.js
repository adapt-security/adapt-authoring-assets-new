...
class Assetsmodule extends AbstractApiModule {
  ...
  async generateThumbnail(assetData) {
    ...
    return new Promise(async (resolve, reject) => {
      const ff = ffmpeg(await this.getRepo(assetData.repo).read(assetData.path))
        .on('error', e => reject(e))
        .on('end', () => resolve())
      ...
      ff.save(this.getThumbPath(assetData))
    });
  }
  /**
  ...
  async checkAssets() {
    ...
    const errors = (await Promise.allSettled(assets.map(async a => {
      try {
        ...
        if(e.code !== this.app.errors.NOT_FOUND) throw e;
        throw this.app.errors.NOT_FOUND.setData({ type: 'asset', _id: a._id.toString() });
      }
    }))).filter(r => r.status === 'rejected').map(r => r.reason);
    if(errors.length) this.log('error', this.app.errors.MISSING_ASSETS.setData({ errors: errors.map(e => e.toString()) }))
  }
  /**
