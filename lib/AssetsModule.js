...
class Assetsmodule extends AbstractApiModule {
  ...
  async init() {
     ...
     */
    this.assetTypes = {};
    this.registerAssetType(LocalAsset);
    ...
    this.app.onReady()
      .then(this.checkAssets.bind(this))
      .then(this.regenerateThumbnails.bind(this));
  }
  /** @override */
  async setValues() {
    this.root = "assets";
  ...
  /**
   * Creates a new asset wrapper for file system operations
   * @param {object} assetData The database data
   * @returns {AbstractAsset}
   */
  createAsset(assetData, ...args) {
    const AssetType = this.assetTypes[assetData.repo];
   ...
   */
  async checkAssets() {
    const errors = (await Promise.allSettled((await this.find()).map(async a => await this.createAsset(a).ensureExists())))
      .filter(r => r.status === 'rejected')
      .map(r => r.reason.toString());

    if(errors.length) this.log('error', this.app.errors.MISSING_ASSETS.setData({ errors }))
  }
  /**
   * Reenerates any missing thumbnails
   */
  async regenerateThumbnails() {
    return Promise.allSettled((await this.find()).map(async a => this.createAsset(a).generateThumb()));
  }
  /**
  ...
  async serveAssetHandler(req, res, next) {
    try {
      ...
      const asset = this.createAsset(assetData);
  ...
  async insert(data, options, mongoOptions) {
    ...
    try {
      const updateData = await this.createAsset(doc).updateFile(data.file, { deleteOnError: true });
      return super.update({ _id: doc._id }, updateData, options, mongoOptions);
    } catch(e) {
  ...
  async update(query, data, options, mongoOptions) {
    ...
    const updateData = await this.createAsset(doc).updateFile(data.file);
  ...
  async delete(query, options, mongoOptions) {
    ...
    const asset = this.createAsset(doc);
