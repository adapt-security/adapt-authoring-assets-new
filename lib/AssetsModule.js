...
class Assetsmodule extends AbstractApiModule {
  ...
  async updateAsset(assetDoc, file, options = { deleteOnError: false }) {
    ...
    try {
      ...
      await repo.write(await this.localRepo.read(file.filepath), file.newFilename);
      await repo.delete(file.filepath);
      ...
      await repo.move(file.newFilename, assetData.path);
  ...
  async replace(query, data, options, mongoOptions) {
    const doc = await super.replace({ _id: query._id }, data, options, mongoOptions);
    return this.updateAsset(doc, data.file);
  }
  /** @override */
