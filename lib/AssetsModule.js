...
class Assetsmodule extends AbstractApiModule {
  ...
  async generateThumbnail(assetData) {
    ...
    return new Promise(async (resolve, reject) => {
      ffmpeg(await this.doRepoAction(assetData.repo, 'read', assetData.path))
   ...
   */
  async doRepoAction(repoName, action, ...args) {
    ...
    return repo[action].call(repo, ...args);
  }
  /**
  ...
  async handleAssetUpload(assetData) {
    ...
    await this.doRepoAction(repo, 'write', fsCallbacks.createReadStream(assetData.file.filepath), path);
  ...
  async serveAssetHandler(req, res, next) {
    ...
    const fileStream = req.query.thumb === "true" ? 
      ...
      await this.doRepoAction(asset.repo, 'read', asset.path);
  ...
  async insert(data, options, mongoOptions) {
    ...
    await this.doRepoAction(doc.repo, 'move', data.file.filepath, newPath);
  ...
  async delete(query, options, mongoOptions) {
    ...
    await this.doRepoAction(doc.repo, 'delete', doc.path);
