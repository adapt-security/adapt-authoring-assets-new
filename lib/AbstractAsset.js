...
class AbstractAsset {
   ...
   * @returns {String}
   */
  getFileExtensionFromSubtype(subtype) {
    switch(subtype) {
  ...
  async generateThumb(options = { regenerate: false }) {
    ...
    return new Promise(async (resolve, reject) => {
      ...
      if(this.isVideo) ff.screenshots({ size, timemarks: ['5'], filename: this.thumb.path });
    });
  }
  /**
  ...
  async updateFile(file) {
    ...
    await this.delete();
    this.setData({ 
      path: `${this.data._id}.${this.getFileExtensionFromSubtype(subtype)}`, 
      repo: this.data.repo, 
      size: file.size,
      subtype, 
      type,
