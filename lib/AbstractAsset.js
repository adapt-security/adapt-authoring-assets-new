...
class AbstractAsset {
  ...
  async generateThumb(options = { regenerate: false }) {
    ...
    return new Promise(async (resolve, reject) => {
      ff.on('end', () => resolve());
      ff.on('error', error => reject(App.instance.errors.GENERATE_THUMB_FAIL.setData({ file: this.data.path, error })));

      const size = `${this.assets.getConfig('thumbnailWidth')}x?`;

      if(this.data.type === 'image') ff.size(size).save(this.thumb.data.path);
      if(this.data.type === 'video') ff.screenshots({ size, timemarks: ['5'], filename: this.thumb.data.path });
    });
  }
  /**
